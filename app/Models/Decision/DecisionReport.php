...
class DecisionReport extends Model {
	...
	public function getForSavedBookDatatable($params, $year, $month) {
		...
		foreach($records as $index => $record) {
			...
			*/
			...
			$reporter = User::where('id', $record->creator)->first();
			if(isset($reporter))
				$newArr[$newindex]['user'] = $reporter->realname;
			else
	...
	public function getForDatatable($params, $status = null) {
		...
		foreach($records as $key => $item) {
			...
			$reporter = User::where('id', $item->creator)->first();
			if(isset($reporter))
				$records[$key]->realname = $reporter->realname;
			else
				$records[$key]->realname = '';
